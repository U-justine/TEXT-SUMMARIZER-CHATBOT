...
def init_database():
    """Initialize SQLite database for users and history"""
...
def save_session(user_id: int, session_name: str, pdf_name: str, extracted_text: str, summary: str = "") -> int:
    """Save user session to database"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO user_sessions (user_id, session_name, pdf_name, extracted_text, summary)
        VALUES (?, ?, ?, ?, ?)
    """, (user_id, session_name, pdf_name, extracted_text, summary))
...
def save_qa(session_id: int, question: str, answer: str, confidence: float = 0.0):
...
def get_user_sessions(user_id: int) -> List[Dict]:
    ...
    cursor.execute("""
        SELECT id, session_name, pdf_name, created_at
        FROM user_sessions
        WHERE user_id = ?
...
def get_session_data(session_id: int) -> Optional[Dict]:
    ...
    cursor.execute("""
        SELECT session_name, pdf_name, extracted_text, summary
        FROM user_sessions
        WHERE id = ?
    """, (session_id,))
...
def get_session_qa_history(session_id: int) -> List[Dict]:
...
def show_sidebar():
    ...
    with st.sidebar:
        ...
        if st.session_state.get('authenticated'):
                ...
                for session in user_sessions:
                    ...
                    if st.button(
                        ...
                        if session_data:
                            ...
                            st.session_state.qa_history = get_session_qa_history(session['id'])
                            st.success(f"📂 Loaded session: {session['session_name']}")
                            st.rerun()

                    st.markdown(f'<div class="history-date">{session_date}</div>', unsafe_allow_html=True)
                    st.markdown("---")
            else:
            ...
            st.metric("📄 Documents Processed", len(user_sessions))

            total_qa = sum(len(get_session_qa_history(session['id'])) for session in user_sessions)
            st.metric("❓ Questions Asked", total_qa)
                    ...
                    with st.spinner("🤖 AI is analyzing your document..."):
                        ...
                        if st.session_state.current_session_id:
                            ...
                            cursor.execute(
                                ...
                                (summary, st.session_state.current_session_id)
                            )
                            conn.commit()
                            conn.close()
